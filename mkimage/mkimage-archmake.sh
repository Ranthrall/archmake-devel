...
expect <<EOF
	...
	spawn pacstrap -C ./mkimage-arch-pacman.conf -c -d -G -i $ROOTFS base make libunistring gcc haveged --ignore $PKGIGNORE
	expect {
...
tar --numeric-owner --xattrs --acls -C $ROOTFS -c . | docker import - acidhub/archmake:latest
docker run -t acidhub/archmake:latest echo Success.
rm -rf $ROOTFS
